...
#figure(
  {
    stack(
        ...
        text(size: 22pt)[Handgloves],
      )
  },
  caption: [Palatino. Make sure the live text (bottom) uses the same font as the image (top).],
) <fig:Palatino>

== Body text

Body text is set in the regular weight at 11 points with custom line spacing of 1.26, and 8.5 points of spacing added after each paragraph. It should be justified and paragraphs should not be indented. These styles can be automatically applied using the _Normal_ paragraph style.

*Bold* and _italics_ should be used for emphasis. Hyperlinks may be inserted in the text, as well as `in-line code`, superscripts#super[ like this], and subscripts#sub[ like this].
...
The author's name and email should come next unless you want to or were asked to submit anonymously, in which case this can be omitted. They should be set in the same size and weight as body text, centered. These styles can be applied using the _Title_ and _Subtitle_ paragraph styles.

== Abstract

If your paper requires an abstract, it should be placed at the top of the first page underneath the title block, preceded by the word *_Abstract_* in bold italics, followed by an em dash. An extra 0.5" indent should be added to both sides. Not all papers require abstracts; only those that would benefit from a high-level summary of the project or its background.

== Headings

Headings should all be set in the same size as body text (11 pt) in bold. With the exception of *_Heading 1_*, they should all have 8.5 pt of space before and after. They should be hierarchically numbered: Word, Pages, and LaTeX will do this automatically when you use the appropriate paragraph styles, but Google Docs users will need to number their headings manually. Headings should not span more than one line.

=== Heading 1

_Heading 1_ should be set in all caps. It should have 11 points of space added before and 8.5 points of space added after

=== Headings 2–4

Besides _Heading 1_, which is set in caps, headings should always use sentence case (i.e., first word capitalized) rather than title case; after all, they are not titles. _Heading 2_ should be set in bold roman (upright), and _Heading 3_ should be set in bold italics. The use of headings beyond _Heading 3_ is discouraged.

#h4[Heading 4]_Heading 4_ is provided as a run-in sidehead. Like _Heading 3_, it is set in bold italics, but it should be followed by an em dash and flow right into the text, as seen at the beginning of the current paragraph. It should be used more as a list style than a heading style, e.g. to set off a list of principles in a heuristic evaluation.

== Page layout

JDF uses the US Letter paper size (8.5" × 11"). It has a top margin of 1", and bottom and side margins of 1.5". This yields a text block of 5.5" × 8.5", which is exactly $1 / 2$ the size of the page, divided lengthwise.
...
You have freedom to format tables in the way that works best for your data. Generally, text should be left-aligned and numbers should be right-aligned or aligned at the decimal – you can do this using a #link("https://practicaltypography.com/tabs-and-tab-stops.html")[custom tab stop]. The default table style (shown below) reduces the text size to be equal to the caption text.
...
#figure(
  kind: table,
  {
    ...
    table(
      columns: (0.25fr, auto, auto, 0.52fr),
      ...
      toprule,
      [Golden ratio], [$phi$], [1.618], [Number such that the ratio of 1 to the number is equal to the ratio of its reciprocal to 1],
      midrule,
  ...
  caption: [Mathematical constants. Notice how the approximations *do not* align at the decimal.],
) <table:Example>
...
You may optionally move certain information to appendices at the end of your paper, after the reference list. If you have multiple appendices, you should create a section with a _Heading 1_ of "Appendices." Each appendix should begin with a descriptive _Heading 2_; appendices can thus be referenced in the body text using their heading number and description, e.g. "Appendix 5.1: Survey responses." If you have only one appendix, you can label it with the word "Appendix" followed by a descriptive title, e.g., "Appendix: Survey responses."

These appendices do not count against the page limit, but they should not contain any information required to answer the question in full. The body text should be sufficient to answer the question, and the appendices should be included only for you to reference or to give additional context. If you decide to move content to an appendix, be sure to summarize the content and note it in relevant place in the body text, e.g., "The raw data can be viewed in _Appendix 5.1: Survey responses_."
